  ...
  return (
    <div className={`relative ${className}`}>
        ...
        {state.displayedTexts.map((paragraph, idx) => (
          ...
          >
            ...
            {idx === state.displayedTexts.length - 1 && !state.isComplete && (
              <span className="inline-block w-0.5 h-4 bg-orange-600 ml-0.5 animate-pulse align-middle" />
            )}
          </motion.p>
        ))}
      </AnimatePresence>
      {!state.isComplete && state.displayedTexts.length > 0 && (
        <button
          onClick={handleSkip}
          className="mt-2 flex items-center gap-1 text-xs text-orange-700/60 hover:text-orange-700 transition-colors"
